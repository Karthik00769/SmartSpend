...
describe('Dashboard Service Aggregation', () => {
  it('aggregates dashboard data and applies FinanceCore math correctly', async () => {
    ...
    vi.spyOn(dbModule, 'query').mockImplementation(async (sql: string) => {
      if (sql.includes('SELECT monthly_income_minor')) {
        return [{ monthly_income_minor: '5000000' }]; // 50,000 INR
      }
      if (sql.includes('SELECT COALESCE(SUM(amount_minor), 0) AS total_spent FROM expenses')) {
      ...
      if (sql.includes('SELECT e.id, e.amount_minor, DATE_FORMAT')) {
        return [{
          id: 1,
          amount_minor: 50000,
          date: '2024-10-01',
    ...
    expect(summary.totalSpentMinor).toBe(2000000);
    expect(summary.totalIncomeMinor).toBe(5000000);
    expect(summary.savingsMinor).toBe(3000000); // 50000 - 20000
    ...
    expect(summary.topCategories[0].spentMinor).toBe(1000000); // from db query
    
    // Recent expenses
    expect(summary.recentExpenses.length).toBe(1);
    expect(summary.recentExpenses[0].amountMinor).toBe(50000);
