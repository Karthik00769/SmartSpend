  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-14 bg-background/95 backdrop-blur
      ...
      <div className="flex items-center gap-3">
        <span className="text-lg font-bold text-foreground bg-clip-text text-transparent bg-gradient-to-r from-primary to-accent">
          {pageTitle}
        </span>
      </div>
...
export function MobileBottomTabBar() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-16 bg-background/95 backdrop-blur border-t border-border z-40 flex items-center justify-around px-2 pb-[env(safe-area-inset-bottom)]">
      {NAV_LINKS.slice(0, 5).map((link) => {
        ...
        return (
          ...
          >
            ...
            <span className="text-[10px] font-medium leading-none truncate w-full text-center">{link.label}</span>
          </Link>
        );
      })}
    </nav>
